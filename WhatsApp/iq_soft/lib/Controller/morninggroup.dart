...
class MorningGroupsController extends GetxController {
  RxBool isloading = true.obs, isSend = false.obs;

  TextEditingController search = TextEditingController();
  TextEditingController customMorning = TextEditingController();
  ...
  morninggroups(String id, context) {
    customMorning.text = Root.morningmessage;
    showGeneralDialog(
      context: context,
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: CustomTextField(
                              controller: customMorning, hint: "info4".tr),
                        ),
                        Padding(
  ...
  confirm(String id, String message) async {
    Root.connect!.chat.sendTextMessage(phone: id, message: message);
    Get.back();
  }
  ...
  filterData(String search) {
    groups.assignAll(filteredGroups
        .where(
            (group) => group.name!.toLowerCase().contains(search.toLowerCase()))
        .toList());
    update();
  }
  ...
  sendMorning() {
    if (isSend.value == false) {
      ...
      for (int i = 0; i <= groups.length; i++) {
        Root.connect!.chat.sendTextMessage(
            phone: groups[i].id!, message: Root.morningmessage);
      }
    }
  }
}
